...
a_step =1 
# choose theta size
a =np.arange(0, mag_im.shape[0], a_step)
b=np.arange(0, mag_im.shape[1], b_step)


acc_mat = []
# accumulation matrix
acc_mat = np.zeros((a.shape[0],b.shape[0],r.shape[0]))
...
circles = cv2.HoughCircles(im, cv2.HOUGH_GRADIENT, 1,
                           10, param1=100, param2=8, minRadius=5, maxRadius=30).astype(int)
